...
func TestChannel_IngestTx_latestUnauthorizedDeclTx(t *testing.T) {
	...
	require.NoError(t, err)
	cs, err := initiatorChannel.ChannelState()
	require.NoError(t, err)
	require.Equal(t, ChannelStateClosing, cs)
...
func TestChannel_IngestTx_oldDeclTx(t *testing.T) {
	...
	cs, err := initiatorChannel.ChannelState()
	require.NoError(t, err)
	require.Equal(t, ChannelStateClosingWithOutdatedState, cs)
}
...
func TestChannel_IngestTx_updateChannelState_nativeAsset(t *testing.T) {
	...
	cs, err := initiatorChannel.ChannelState()
	require.NoError(t, err)
	assert.Equal(t, ChannelStateNone, cs)
	open, err := initiatorChannel.ProposeOpen(OpenParams{
		ObservationPeriodTime:      1,
		ObservationPeriodLedgerGap: 1,
		ExpiresAt:                  time.Now().Add(time.Minute),
	...
	cs, err = initiatorChannel.ChannelState()
	require.NoError(t, err)
	assert.Equal(t, ChannelStateOpen, cs)
...
func TestChannel_IngestTx_updateChannelState_nonNativeAsset(t *testing.T) {
	...
	cs, err = initiatorChannel.ChannelState()
	require.NoError(t, err)
	assert.Equal(t, ChannelStateFailed, cs)
...
func TestChannel_IngestTx_updateChannelState_invalid_initiatorEscrowHasExtraSigner(t *testing.T) {
