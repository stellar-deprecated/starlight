...
func TestChannel_IngestTx_updateOpenState_nonNativeAsset(t *testing.T) {
	...
	err = initiatorChannel.ingestFormationTx(resultMetaXDR)
	require.NoError(t, err)
	assert.EqualError(t, initiatorChannel.openExecutedWithError, "incorrect signer weights found")
	assert.Equal(t, OpenStateFailed, initiatorChannel.OpenState())
	...
	err = initiatorChannel.ingestFormationTx(resultMetaXDR)
	require.NoError(t, err)
	assert.EqualError(t, initiatorChannel.openExecutedWithError, "incorrect number of signers, found: 3 want: 2")
	assert.Equal(t, OpenStateFailed, initiatorChannel.OpenState())
