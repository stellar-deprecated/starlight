...
func TestChannel_IngestTx_updateOpenState_nonNativeAsset(t *testing.T) {
	...
	err = initiatorChannel.ingestFormationTx(resultMetaXDR)
	require.EqualError(t, err, "incorrect initiator escrow account sequence number found, found: 24936580120578 want: 24936580120577")
	assert.Equal(t, OpenStateFailed, initiatorChannel.OpenState())
	assert.Equal(t, err, initiatorChannel.formationTxError)
	...
	err = initiatorChannel.ingestFormationTx(resultMetaXDR)
	require.EqualError(t, err, "incorrect signer weights found")
	assert.Equal(t, OpenStateFailed, initiatorChannel.OpenState())
	assert.Equal(t, err, initiatorChannel.formationTxError)
	...
	err = initiatorChannel.ingestFormationTx(resultMetaXDR)
	require.EqualError(t, err, "incorrect number of signers, found: 3 want: 2")
	assert.Equal(t, OpenStateFailed, initiatorChannel.OpenState())
	assert.Equal(t, err, initiatorChannel.formationTxError)
	...
	err = initiatorChannel.ingestFormationTx(resultMetaXDR)
	require.EqualError(t, err, "incorrect initiator escrow account thresholds found")
	assert.Equal(t, OpenStateFailed, initiatorChannel.OpenState())
	assert.Equal(t, err, initiatorChannel.formationTxError)
	...
	err = initiatorChannel.ingestFormationTx(resultMetaXDR)
	require.EqualError(t, err, "incorrect trustline asset found for nonnative asset channel")
	assert.Equal(t, OpenStateFailed, initiatorChannel.OpenState())
	assert.Equal(t, err, initiatorChannel.formationTxError)
	...
	err = initiatorChannel.ingestFormationTx(resultMetaXDR)
	require.EqualError(t, err, "incorrect trustline flag, needs to be authorized")
	assert.Equal(t, OpenStateFailed, initiatorChannel.OpenState())
	assert.Equal(t, err, initiatorChannel.formationTxError)
}
